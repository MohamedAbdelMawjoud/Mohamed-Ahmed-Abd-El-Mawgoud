...
My main focus is **Machine Learning, Deep Learning, NLP, Computer Vision, Transformers, and Generative AI**.
...
`YOLO11` • `PyTorch` • `ONNX` • `TFLite` • `Optuna` • `FastAPI` • `Docker` • `Azure`
...
**Technologies:**
`Python` • `Scikit-Learn` • `TensorFlow` • `PyTorch` • `DistilBERT` • `Transformers`
...
**Technologies:**
`TensorFlow` • `Keras` • `EfficientNetB0` • `CNN` • `Data Augmentation`
...
**Technologies:**
`Python` • `Pandas` • `Scikit-Learn` • `Data Analysis` • `Machine Learning`
...
**Technologies:**
`Python` • `MFCC` • `Scikit-Learn` • `SVM` • `Random Forest` • `AdaBoost` • `Gradio`
...
* **Quantum Computing Program** — QWorld × Alamein International University
...
**Data → Modeling → Training → Evaluation → Optimization → Deployment**
